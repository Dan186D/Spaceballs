...
inline void io_init(io_t* io, FILE* fp, bool write, char* out_line, int out_line_n, char* out_error, int out_error_n) {
	io->fp = fp;
	io->write = write;
	io->line = out_line;
...
#define io_bounding_box(io, datum, ...) \
io_vec3(io, datum.min) \
&& io_vec3(io, datum.max)

#define io_light(io, datum, ...) \
   io_vec3(io, datum.pos) \
&& io_vec3(io, datum.colour) \
&& io_float(io, datum.constant) \
&& io_float(io, datum.linear) \
&& io_float(io, datum.quadratic) \
&& io_bool(io, datum.on)

#define io_level_geo(io, datum, ...) \
   io_string(io, datum.model_file_name, CUTE_FILES_MAX_FILENAME) \
&& io_string(io, datum.tex_file_name, CUTE_FILES_MAX_FILENAME) \
&& io_vec2(io, datum.tex_scale) \
&& io_vec2(io, datum.tex_offset) \
&& io_vec3(io, datum.pos_m) \
&& io_vec3(io, datum.scale) \
&& io_vec3(io, datum.rot_axis) \
&& io_float(io, datum.angle) \
&& io_bool(io, datum.collision)

#define io_enemy_spawner(io, datum, ...) \
   io_bool(io, datum.active) \
&& io_vec3(io, datum.pos_m) \
&& io_bounding_box(io, datum.bb) \
&& io_uint8(io, datum.num_enemy[0]) \
&& io_uint8(io, datum.num_enemy[1]) \
&& io_uint8(io, datum.num_enemy[2]) \
&& io_uint8(io, datum.num_enemy[3])

#define io_enemy_spawner_new(io, datum, ...) \
   io_bool(io, datum.active) \
&& io_vec3(io, datum.pos_m) \
&& io_bounding_box(io, datum.bb) \
&& io_uint8(io, datum.num_enemy[0]) \
...
&& io_uint8(io, datum.num_enemy[2]) \
&& io_uint8(io, datum.num_enemy[3]) \
&& io_uint8(io, datum.num_enemy[4])

#define io_text_trigger_new(io, datum, ...) \
   io_bool(io, datum.active) \
&& io_bounding_box(io, datum.bb) \
&& io_vec3(io, datum.text_pos) \
&& io_vec3(io, datum.normal_dir) \
&& io_float(io, datum.angle) \
&& io_string(io, datum.str, MAX_TEXT_LENGTH)
